...
parser.add_argument('--m_b', type=int, default=200)
...
def main():
    ...
    for j in range(args.idx, args.idx + args.nrep):
        ...
        training_data = DTL_class.gen_train_data(ntrain=ntrain, n_b=n_b, m_b=m_b, basis_type=args.basis_type,
                                                 remove_D0=args.indep)
        ...
        #     print(torch.sum(torch.argmax(Z_vae, 1) == win_idx) / n_vae)
        ...
        gamma_list = np.linspace(-20 / np.sqrt(n_b + m_b), 20 / np.sqrt(n_b + m_b), 201)
        ...
        print("covered_nn", logs[j - args.idx]['covered_nn'])
        ...
        for ell in range(int(nb / np.mean(W_train))):
            ...
            if idx != win_idx:
                ...
                target_theta_0 = target_theta_0.reshape(1, len(gamma_list))
                ...
                # true weight
                weight_val_true = norm.sf((np.sort(X_bar)[-2] - theta_data - gamma_list) / np.sqrt(var_0))
                weight_val_2 = weight_val_true * norm.pdf((target_val - observed_target) / target_sd)
                exp_family = discrete_family(target_val.reshape(-1), weight_val_2.reshape(-1))
                pivot = exp_family.cdf((hypothesis - observed_target) / target_var, x=observed_target)
                pivot = 2 * min(pivot, 1 - pivot)
                pval_true.append(pivot)

                pivot_naive = norm.cdf((hypothesis - observed_target) / target_sd)
                pval_naive.append(2 * min(pivot_naive, 1 - pivot_naive))
                if count == nb:
                    break
        pval = np.array(pval)
        ...
        cdf_naive = np.zeros(len(grid))
        cdf_true = np.zeros(len(grid))
        for ii in range(len(grid)):
            cdf[ii] = np.mean(pval <= grid[ii])
            cdf_naive[ii] = np.mean(pval_naive <= grid[ii])
            cdf_true[ii] = np.mean(pval_true <= grid[ii])
        plt.plot(grid, cdf, label='nn', lw=3)
        plt.plot(grid, cdf_naive, label='naive', ls='--', lw=3)
        # plt.plot(grid, cdf_true, label='true')
        plt.plot(grid, grid, ls=':', lw=3)
        ...
        print('covered_naive', logs[j - args.idx]['covered_naive'])
